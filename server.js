...
for (const key in process.env) {
    if (Object.hasOwnProperty.call(process.env, key) && key.toLowerCase().startsWith('ptu')) {
        ...
        resp.on('end', () => {
            try {
                ...
                if(Array.isArray(data)){
                    appendTemplate(data)
                }else if(Object.keys(data).includes('templates') && Array.isArray(data.templates)){
                    appendTemplate(data.templates)
                }
            } catch (e) {
                console.log(e)
            }
